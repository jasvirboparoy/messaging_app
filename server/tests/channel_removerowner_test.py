...
from src.channel import channel_invite , channel_join, channel_details, channel_addowner, channel_removeowner
...
# Assume channel_addowner is working
# Assume auth_register works
# assume auth_logout works
#two functions to get a new user and new channel

# in case of conflicting errors assume accesserror is raised then input error
@pytest.fixture
def get_5_new_users():
    reset_workspace()
...
def test_channel_removeowner_basic(setup_channel):
    ...
    channel_removeowner(user[0]["token"], c_id, user[1]["u_id"])

    # Assert user 1 is still an owner and user 2 not an owner
    owner_members = channel_details(user[0]["token"], c_id)["owner_members"]
    owner_members_u_ids = [member['u_id'] for member in owner_members]
    assert user[1]["u_id"] not in owner_members_u_ids
    assert user[0]["u_id"] in owner_members_u_ids
    reset_workspace()

def test_channel_removeowner_multiple(setup_channel):
    # Tests for basic functionality by removing multiple users from original user persepective
    ...
    # Remove the newly added owners
    channel_removeowner(user[0]["token"], c_id, user[1]["u_id"])
    channel_removeowner(user[0]["token"], c_id, user[2]["u_id"])
    channel_removeowner(user[0]["token"], c_id, user[3]["u_id"])

    # Assert that removed owners are not owners in the same channel
    owner_members = channel_details(user[0]["token"], c_id)["owner_members"]
    owner_members_u_ids = [member['u_id'] for member in owner_members]
    
    assert user[3]["u_id"] not in owner_members_u_ids
    assert user[2]["u_id"] not in owner_members_u_ids
    assert user[1]["u_id"] not in owner_members_u_ids
    assert user[0]["u_id"] in owner_members_u_ids
    reset_workspace()

def test_channel_removeowner_multiple2(setup_channel):
    # Tests for basic functionality by removing a random combination of multiple users from 
    # the perspective of different owners

    user, c_id = setup_channel

    channel_addowner(user[0]["token"], c_id, user[1]["u_id"])
    channel_addowner(user[1]["token"], c_id, user[2]["u_id"])
    channel_addowner(user[2]["token"], c_id, user[3]["u_id"])
    channel_addowner(user[3]["token"], c_id, user[4]["u_id"])

    # remove two owners from two current owners
    channel_removeowner(user[3]["token"], c_id, user[4]["u_id"])
    channel_removeowner(user[2]["token"], c_id, user[3]["u_id"])
    
    # Assert that that the right combination of users are owners
    owner_members = channel_details(user[0]["token"], c_id)["owner_members"]
    owner_members_u_ids = [member['u_id'] for member in owner_members]
    
    assert user[4]["u_id"] not in owner_members_u_ids
    assert user[3]["u_id"] not in owner_members_u_ids
...
def test_channel_removeowner_inputerror(setup_channel):
    #tests for inputerrors

    user, c_id = setup_channel

    # add 1 users as owner
    channel_addowner(user[0]["token"], c_id, user[1]["u_id"])

    # Assume that -111111 and -111113 is not a valid channel id
    with pytest.raises(InputError) as e:
        channel_removeowner(user[0]["token"], -111111, user[1]["u_id"])
    
    with pytest.raises(InputError) as e:
        channel_removeowner(user[0]["token"], -111113, user[1]["u_id"])

    # Try to remove owners which are not owners of the channel
...
def test_channel_removeowner_accesserror(setup_channel):
    #tests for accesserrors

    user, c_id = setup_channel

    # Accesserror when user[1] tries to remove themself or another user as an owner
    # user[1] is not an owner

    # tries to remove an owner
    with pytest.raises(AccessError) as e:
        channel_removeowner(user[1]["token"], c_id, user[0]["u_id"])
    
    with pytest.raises(AccessError) as e:
        channel_removeowner(user[2]["token"], c_id, user[0]["u_id"])

    with pytest.raises(AccessError) as e:
        channel_removeowner(user[3]["token"], c_id, user[0]["u_id"])
